...
선형모형(affine) 도 컨벡스최적화 대상이 되기 때문이다. 

CVXPY를 이용해서 선형모형 제한조건 최적화를 해보자. 
...
그리고 알고리듬 안에 $x_{1} \ge 0, x_{2} \ge 0$ 은 기본 적용되어 있었다. 따로 지정하지 않아도 된다. 

# CVXOPT 이용해서 선형계획법 문제 풀기 

선형모형은 affine 함수로, 컨벡스 필요조건을 만족한다. (DCP rule)

따라서 선형모형도 컨벡스 최적화 대상이 될 수 있다. 

선형계획법은 선형모형을 제한조건 걸고 최적화 하는 작업이었다.

$\Rightarrow$ 선형계획법은 제한조건 있는 컨벡스 최적화 문제로도 풀 수 있다. 

CVXPY 말고 CVXOPT 도 컨벡스 최적화를 위한 패키지였다. 

CVXOPT 에는 선형계획법 문제를 해결할 수 있는 solver.lp() 명령이 있다. 
...
마찬가지로 알고리듬 안에 $x_{1} \ge 0, x_{2} \ge 0$ 은 기본 적용되어 있어서 따로 지정 안 해줘도 된다.
...
알고리듬 안에 $x_{1} \ge 0 , x_{2} \ge 0$ 은 기본으로 적용되어 있다.
...
앞에서 목적함수 $x_{1}^{2}+x_{2}^{2}$ 가 일반화된 이차형식(quadratic form) 으로 바꿀 수 있다는 걸 봤다. 
...
다만, 알고리듬 안에서는 위 기본 조건이 이미 적용되어 있다. 따로 입력하지 않아도 된다. 
