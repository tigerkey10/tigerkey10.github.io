...
- $X = x_{i}$ 일 때 $Y$ 확률변수 조건부엔트로피를 $X$ 주변확률분포값으로 가중합 한 것이다. 
...
$H[Y\vert X] = -\int_{y}p(y\vert x)\log_{2}{p(y\vert x)}dx$
...
위 블로그의 크로스엔트로피 설명을 통해 개념을 확실히 이해할 수 있었다. 
