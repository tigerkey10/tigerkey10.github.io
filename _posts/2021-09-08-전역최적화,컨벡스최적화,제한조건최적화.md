...
## 따라서 컨벡스 최적화는 전역 최적화 문제를 푸는 도구가 될 수도 있다. 
...
- 컨벡스함수, 컨케이브 함수, affine 함수, constant 함수에 대해 컨벡스 최적화 할 수 있는 패키지다. 
...
$h_{1} = x_{1}+x_{2} + \frac{\sqrt{2}}{2}(x^{2}_{1}+x_{2}^{2}-1)$

$h_{2} = x_{1}+x_{2}-\frac{\sqrt(2)}{2}(x^{2}_{1}+x_{1}^{2}-1)$
