...
함수 전 구간에 대해 최적화 해서, (반드시) 전역 최적해를 찾아내는 최적화 방법
...
DCP rule 을 만족하면 무조건 컨벡스 함수다. 

---

한편 DCP rule 을 만족하지 않지만, 컨벡스 함수인 경우들도 존재한다. 
...
전역에서 언제나 전역 최적해로 수렴이 가능하고. 모양도 아래로 볼록한. 컨벡스 함수임을 관찰할 수 있다. 
...
한편 대부분의 경우 최적화 할 때 최소화 문제만을 고려한다. 최소화 문제를 풀 수 있으면 최대화 문제도 풀 수 있기 때문이다. 

그러면 컨벡스 최적화의 최적화 문제도 대부분의 경우 $Minimize(Convex)$ 일 것이다. 

## 그래서 컨벡스 최적화를 다음과 같이 정의하기도 한다. 

## $\frac{d^{2}h}{dx^{2}} \ge 0$

'목적함수의 2차 도함수가 0 이상인 구간 에서만 정의된 최적화'

다변수함수의 경우 

## $x^{T}Hx \ge 0$

'목적함수의 헤시안 행렬(2차 도함수 행렬)이 양의 준정부호인 영역 에서만 정의된 최적화'

## 즉, 목적함수 $h$의 볼록구간(영역)에서만 정의된 최적화라는 것이다. 
...
둘 다 컨벡스 최적화를 위한 파이썬 패키지다. 


## CVXPY 
### DCP rule 을 만족하는. 컨벡스 함수.만 컨벡스 최적화 하는 패키지다.
- 직관적 심볼릭 연산 할 수 있다는 장점, 특징이 있다.
- 컨벡스함수, 컨케이브 함수, affine 함수, constant 함수에 대해 컨벡스 최적화 할 수 있다.
...
- Convex 함수 이외에 DCP rule 을 만족하는 Concave, Affine, Constant 함수도 컨벡스 최적화가 가능했다.
...
CVXPY로 컨벡스 최적화 하기위해서는 

목적함수와 제한조건이 컨벡스 필요조건. DCP rule 을 만족해야만 한다. 

CVXPY 문서에 보면 DCP rule 에 대한 상세설명이 있다. 

https://www.cvxpy.org/tutorial/dcp/index.html#dcp-problems

## 1. DCP rule 을 만족하는 목적함수는 ...

다음 두 가지 형태 중 하나여야 한다. 
...
$\Leftarrow$ 실제로 affine, constant 함수는 convex, concave 함수가 아닌데 DCP rule 을 만족했다. 이 두 함수는 함수 전역에서 최소화, 최대화가 가능하다.

## 2. DCP rule 을 만족하는 제한조건은 ...
...
## 3. 목적함수, 최적화 문제가 DCP rule 만족하는지 알 수 있는 방법
...
우선 목적함수 obj2 는 $affine$ 으로 분류된다. 컨벡스 최적화 대상이다. 
