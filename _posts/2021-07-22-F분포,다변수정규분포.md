...
- D차원 입력벡터 만들어내는 확률변수 x가 있다. 이 확률변수 x는 N(X|$\mu$, $\Sigma$) 다변수 정규분포를 따른다고 하자. 
...
- 우리가 찾는 건 $p(X_{a} | X_{b})$ 의 확률분포다. 

- 여기서 현재의 결합확률분포 $p(X)$ = $p(X_{a}|X_{b})$ 로 나타낼 수 있다. 
...
- 두번 미분한 값(2차 항의 계수) 은 공분산의 역행렬과 같았다. 따라서, 조건부분포 $p(X_{a}|X_{b})$ 의 공분산은 다음과 같다. 
...
- 위 식은 앞에서 계산한 $\Sigma_{a|b}^{-1} \mu_{a|b}$ 와 같아져야 한다. 

- 식을 전개하면 다음과 같다. 

$\Sigma_{a|b}^{-1}$ = {$\Lambda_{aa}\mu - \Lambda_{ab}(X_{b}-\mu_{b})$}

--> $\mu_{a|b}$ = $\Sigma_{a|b}${$\Lambda_{aa}$ $\mu_{a}$ $-$ $\Lambda_{ab} (X_{b}-\mu_{b})$}

--> 앞에서 ($\Sigma_{a|b}$ = $\Lambda_{aa}^{-1}$) 이었다. 적용하면

--> $\mu_{a|b}$ $=$ $\Lambda_{aa}^{-1}${$\Lambda_{aa}\mu_{a} - \Lambda_{ab} (X_{b} - \mu_{b})$}

- $\mu_{a|b}$ $=$ $\mu_{a} - \Lambda_{aa}^{-1}\Lambda_{ab}(X_{b}-\mu_{b})$ 이 된다. 
...
여기서 얻은 결과를 $p(X_{a}|X_{b})$ 의 평균. 분산에 대입하면

- $\mu_{a|b} = \mu_{a} + \Sigma_{aa}\Sigma_{bb}^{-1}(X_{b}-\mu_{b})$
...
- $\mu_{a|b}$ 는 $X_{b}$ 에 대한 선형함수 꼴이다. 
- $\Sigma_{a|b}$ 는 $X_{b}$ 에 대해 독립적인 식이다. 

- $X_{2}$ 가 어떤 값으로 주어지면 $X_{1}$의 조건부확률분포는 조건부 기댓값 $\mu_{a|b}$, 조건부공분산 행렬 $\Sigma_{a|b}$ 갖는 다변수 정규분포를 따르게 된다. 
...
- 앞에서는 $p(X_{a}, X_{b})$ 분포가 다변수정규분포이면 $p(X_{a} | X_{b})$ 분포도 다변수 정규분포가 된다는 걸 확인했다. 
...
- $N(x_{1}|\mu_{1}+(p\sigma_{1}\sigma_{2}/\sigma_{2}^{2})(x_{2}-\mu_{2}), \sigma_{1}^{2} - ((p\sigma_{1}\sigma_{2})^{2}/\sigma_{2}^{2}))$
...
- $p(x_{1}|x_{2})$ 는 $N(x_{1}|\mu_{1|2}, \Sigma_{1|2})$ 따른다
...
- $\mu_{a|b} = \Lambda_{11}^{-1}(-\Lambda_{21}x_{2}+\Lambda_{11}\mu_{1}+\Lambda_{12}\mu_{2})$
...
- $\Sigma_{a|b} = \Lambda_{11}^{-1}$
- $\mu_{a|b} = \Lambda_{11}^{-1}(-\Lambda_{21}x_{2}+\Lambda_{11}\mu_{1}+\Lambda_{12}\mu_{2})$

= $-\Lambda_{11}^{-1}\Lambda_{21}x_{2}+\mu_{1}+\Lambda_{11}^{-1}\Lambda_{12}\mu_{2}$

= $\mu_{a|b} = \mu_{1} + (\Lambda_{11}^{-1}\Lambda_{12})(\mu_{2}-x_{2})$

- 앞에서 구한 $\Lambda_{11}$ 과 $\Lambda_{12}$ 을 이용해서 $\Sigma_{a|b}$, $\mu_{a|b}$ 식을 바꾸면 다음과 같다. 

- $\mu_{a|b} = \mu_{1}+\Sigma_{12}\Sigma_{22}^{-1}(x_{2}, \mu_{2})$
- $\Sigma_{a|b} = \Sigma_{11}-\Sigma_{12}\Sigma_{22}^{-1}\Sigma_{21}$
...
$\mu_{a|b}$, $\Sigma_{a|b}$ 에 대입해서 식 정리하면 

- $\mu_{a|b} = \mu_{1} + ((p\sigma_{1}\sigma_{2})/\sigma_{2}^{2})$ $(x_{2}-\mu_{2})$

- $\Sigma_{a|b} = \sigma_{1}^{2}-(p\sigma_{1}\sigma_{2})^{2}/\sigma_{2}^{2}$
